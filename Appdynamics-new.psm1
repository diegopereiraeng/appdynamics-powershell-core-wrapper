...
"@)
    ...
    [System.Object] GetRemoteServices ([string]$appID){
        
        $url = $this.baseurl+"/controller/restui/backend/list/remoteService"#+"?output=JSON"
        try
        {
            $start_time= (Get-Date).AddDays(-1) | ConvertTo-UnixTimestamp
            $end_time= Get-Date | ConvertTo-UnixTimestamp # Now
            $body = '{"requestFilter":{"queryParams":{"applicationId":'+$appID+'},"filters":[]},"resultColumns":["ID","NAME","TYPE","CALLS","RESPONSE_TIME","ERRORS"],"offset":0,"limit":-1,"searchFilters":[],"columnSorts":[],"timeRangeStart":'+$start_time+',"timeRangeEnd":'+$end_time+'}'
            $responseData = Invoke-WebRequest -Uri $url -Headers $this.headers -Body $body -WebSession $this.session -Method Post -UseBasicParsing
            $content = $responseData.content
            $remote_services = ($content | ConvertFrom-Json)
            #DEBUG#Write-Host $responseData
    ...
    [bool] PublishAnalyticsEvents ($index,$data){
        ...
        try {
            ...
            $this.Log($source,"ERROR","Removing Special character")
            $this.Log($source,"ERROR",$_.Exception.Message)
            $this.Log($source,"ERROR",$_.Exception.Data)
            $this.Log($source,"ERROR",$_.Exception.InnerException.Message)
            $this.Log($source,"ERROR",$_.Exception.InnerException.Data )
        ...
        try {
            ...
            $this.Log($source,"ERROR","Converting from json")
            $this.Log($source,"ERROR",$_.Exception.Message)
            $this.Log($source,"ERROR",$_.Exception)
            $this.Log($source,"ERROR",$data2)
            return $false
        }
        $events_count = $json.count
        $this.Log($source,"DEBUG","Eventos: $events_count")

        if ($json.count -gt 1) {
            
            foreach ($metricData in $json) {
                if ($bucketCount -gt 698) {
                    ...
                    try
                    {
                        ...
                        $this.Log($source,"DEBUG","Success publishing bucket $buckets_count - Status Code "+[string]$responseData.Response.StatusCode.value__)
                    }
                    catch
                    {
                        $StatusCode = $_.Exception.Response.StatusCode.value__
...
function Start-MultiThreadAppdyJobs {
    ...
    foreach($app in $list){
        ...
        $jobs += Start-ThreadJob -ThrottleLimit 16 -FilePath $script -ArgumentList $app.name, $params
        $count = $count -1
        $count2 += 1
